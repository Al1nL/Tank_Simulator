...
namespace GameManager_212535058_324022904
{
    ...
    GameResult GameManager::run(
        ...
        size_t steps, size_t numofshells,
    ...
    GameResult GameManager::prepareResult() const
    {
        GameResult result;
    ...
    int GameManager::countAliveTanks(int player_id) const
    {
        int count = 0;
        for (const auto &tank : board->getSortedTanks())
        {
            if (tank->getOwnerId() == player_id && !tank->isDestroyed())
            {
                count++;
            }
        }
